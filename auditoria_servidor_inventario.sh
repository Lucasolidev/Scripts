#!/bin/bash
# ==============================================================================
# Script: auditoria_servidor_inventario.sh
# Descrição: Auditoria, varredura e inventário do servidor (não invasivo) para diagnóstico pré-migração
# Autor: Lucas Oliveira
# Repositório: https://github.com/Lucasolidev/Scripts
# Execução recomendada (copiar e colar comando único):
# wget https://raw.githubusercontent.com/Lucasolidev/Scripts/main/auditoria_servidor_inventario.sh -O auditoria_servidor_inventario.sh && sudo chmod +x auditoria_servidor_inventario.sh && sudo ./auditoria_servidor_inventario.sh
# ==============================================================================
...
if [ "$(id -u)" -ne 0 ]; then
    ...
    read -r -p "$(echo -e "  ${FG_YELLOW}${ARROW} Deseja continuar mesmo sem privilégios de root? (s/N): ${NC}")" CONTINUAR_SEM_ROOT || true
    if [[ ! "${CONTINUAR_SEM_ROOT:-}" =~ ^[Ss]$ ]]; then
        log_error "Execução cancelada pelo usuário (privilégios insuficientes)."
        exit 1
    fi
fi
...
LOG_DIR=$(mktemp -d -p /tmp auditoria_inventario.XXXXXX)
LOG_FILE="${LOG_DIR}/${REPORT_FILENAME}"
...
CPU_MODEL=$(grep -m 1 "model name" /proc/cpuinfo 2>/dev/null | cut -d':' -f2 | xargs || echo "Desconhecido")
...
log_item "Memória RAM"         "${MEM_TOTAL:-N/A} Total (Usado: ${MEM_USED:-N/A} | Disponível: ${MEM_AVAIL:-N/A})"
log_item "Memória Swap"        "${SWAP_TOTAL:-N/A}"
...
DEFAULT_GATEWAY=$(ip route 2>/dev/null | awk '/default/ {print $3}' | head -n 1 || true)
DNS_SERVERS=$(grep "nameserver" /etc/resolv.conf 2>/dev/null | awk '{print $2}' | xargs || true)
...
if command -v apache2 > /dev/null 2>&1 || command -v httpd > /dev/null 2>&1; then
    ...
    APACHE_VER=$($APACHE_BIN -v 2>/dev/null | head -n 1 | cut -d':' -f2 | xargs || true)
    log_success "Apache detectado: ${BOLD}${APACHE_VER}${NC} [$(get_service_status apache2)]"
    
    echo -e "    ${BOLD}Módulos Apache Habilitados:${NC}"
    APACHE_MODS=$(apache2ctl -M 2>/dev/null | grep -E "_module" | awk '{print $1}' | sed 's/_module//' | xargs || true)
    [ -z "$APACHE_MODS" ] && APACHE_MODS=$($APACHE_BIN -M 2>/dev/null | grep -E "_module" | awk '{print $1}' | sed 's/_module//' | xargs || true)
    echo -e "    ${DIM}${APACHE_MODS:-Nenhum detectado}${NC}"
    
    echo -e "\n    ${BOLD}VirtualHosts / Sites Ativos no Apache:${NC}"
    APACHE_VHOSTS=$(apache2ctl -S 2>/dev/null | grep -E "port 80|port 443|namevhost|default" | sed 's/^[ \t]*//' || true)
    [ -z "$APACHE_VHOSTS" ] && APACHE_VHOSTS=$($APACHE_BIN -S 2>/dev/null | grep -E "port 80|port 443|namevhost|default" | sed 's/^[ \t]*//' || true)
...
if command -v nginx > /dev/null 2>&1; then
    NGINX_VER=$(nginx -v 2>&1 | cut -d':' -f2 | xargs || true)
    ...
    for conf in /etc/nginx/sites-enabled/* /etc/nginx/conf.d/*.conf; do
        if [ -f "$conf" ]; then
            DOMAINS=$(grep -E "^\s*server_name\s+" "$conf" 2>/dev/null | sed 's/server_name//' | tr -d ';' | xargs || true)
            DOC_ROOT=$(grep -E "^\s*root\s+" "$conf" 2>/dev/null | sed 's/root//' | tr -d ';' | xargs | head -n 1 || true)
...
if command -v php > /dev/null 2>&1; then
    CLI_PHP_VER=$(php -r 'echo PHP_VERSION;' 2>/dev/null || true)
    log_success "PHP CLI Padrão (Linha de Comando): ${BOLD}PHP ${CLI_PHP_VER}${NC}"
    
    # Checa versão ativa no Apache
    APACHE_PHP_MOD=$(find /etc/apache2/mods-enabled/ -maxdepth 1 -name "php*.load" 2>/dev/null | head -n 1 || true)
    ...
    PHP_MODULES=$(php -m 2>/dev/null | grep -v "\[" | sort | xargs || true)
...
if command -v mariadb > /dev/null 2>&1 || command -v mysql > /dev/null 2>&1; then
    ...
    DB_VER=$($DB_BIN --version 2>/dev/null || true)
...
if command -v redis-server > /dev/null 2>&1; then
    REDIS_VER=$(redis-server --version 2>/dev/null | cut -d'=' -f2 | awk '{print $1}' || true)
    ...
    if [ -d "$base_dir" ]; then
        ...
        while IFS= read -r joomla_ver_file; do
            if [ -f "$joomla_ver_file" ]; then
                ...
                J_VER_EXACT=$(php -r "require_once '$joomla_ver_file'; if (class_exists('Joomla\CMS\Version')) { echo \Joomla\CMS\Version::MAJOR_VERSION . '.' . \Joomla\CMS\Version::MINOR_VERSION . '.' . \Joomla\CMS\Version::PATCH_VERSION; } elseif (class_exists('JVersion')) { \$v = new JVersion(); echo \$v->getShortVersion(); }" 2>/dev/null || true)
                
                # Fallback via regex no arquivo Version.php
                if [ -z "$J_VER_EXACT" ]; then
                    MAJOR=$(grep -E "const\s+MAJOR_VERSION|public\s+\$RELEASE" "$joomla_ver_file" 2>/dev/null | head -n 1 | awk '{print $NF}' | tr -d "';\"" || true)
                    MINOR=$(grep -E "const\s+MINOR_VERSION|public\s+\$DEV_LEVEL" "$joomla_ver_file" 2>/dev/null | head -n 1 | awk '{print $NF}' | tr -d "';\"" || true)
                    PATCH=$(grep -E "const\s+PATCH_VERSION|public\s+\$BUILD" "$joomla_ver_file" 2>/dev/null | head -n 1 | awk '{print $NF}' | tr -d "';\"" || true)
                ...
                if [ -f "${APP_DIR}/configuration.php" ]; then
                    J_DB=$(grep "public \$db " "${APP_DIR}/configuration.php" 2>/dev/null | cut -d"'" -f2 || true)
                    J_USER=$(grep "public \$user " "${APP_DIR}/configuration.php" 2>/dev/null | cut -d"'" -f2 || true)
                    J_PREFIX=$(grep "public \$dbprefix " "${APP_DIR}/configuration.php" 2>/dev/null | cut -d"'" -f2 || true)
        ...
        while IFS= read -r wp_ver_file; do
            if [ -f "$wp_ver_file" ]; then
                ...
                WP_VER=$(grep "\$wp_version\s*=" "$wp_ver_file" 2>/dev/null | cut -d"'" -f2 || true)
                log_success "WordPress Detectado!"
                log_item "Caminho" "${FG_CYAN}${APP_DIR}${NC}"
                log_item "Versão"  "${FG_YELLOW}WordPress ${WP_VER:-Desconhecida}${NC}"
                echo -e ""
                FOUND_APPS=$((FOUND_APPS + 1))
            fi
...
if command -v ufw > /dev/null 2>&1; then
    UFW_STATUS=$(ufw status 2>/dev/null | head -n 1 || echo "Inativo")
...
if command -v fail2ban-client > /dev/null 2>&1; then
    ...
    if systemctl is-active --quiet fail2ban 2>/dev/null; then
        JAILS=$(fail2ban-client status 2>/dev/null | grep "Jail list" | cut -d':' -f2 | xargs || true)
...
if command -v docker > /dev/null 2>&1; then
    ...
    docker ps --format "table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null | sed 's/^/    /' || true
else
...
find /etc/cron.d/ -maxdepth 1 -type f 2>/dev/null | awk -F'/' '{print "    • " $NF}' || echo "    • Nenhum arquivo de cron encontrado"
...
# Salva em /root
cp "$LOG_FILE" "/root/${REPORT_FILENAME}" 2>/dev/null || true
cp "$LOG_FILE" "/root/${REPORT_LATEST}" 2>/dev/null || true
...
if [ -n "${SUDO_USER:-}" ] && [ "$SUDO_USER" != "root" ]; then
    ...
    if [ -d "$REAL_USER_HOME" ]; then
        cp "$LOG_FILE" "${REAL_USER_HOME}/${REPORT_FILENAME}" 2>/dev/null || true
        cp "$LOG_FILE" "${REAL_USER_HOME}/${REPORT_LATEST}" 2>/dev/null || true
...
# Salva na pasta do usuário atual se não estiver em sudo
    ...
    cp "$LOG_FILE" "${HOME}/${REPORT_FILENAME}" 2>/dev/null || true
    cp "$LOG_FILE" "${HOME}/${REPORT_LATEST}" 2>/dev/null || true
