...
| Banco | Loopback; credencial root em `/root/credenciais_lemp_<dominio>_<data_hora>.txt`, modo 0600 |
| Diagnóstico | Não cria phpinfo público |
| Relatórios | `/root/relatorio_install_lemp_ubuntu_*.log`, cópia na home sudo e atalho latest; modo 0600 |
...
Validar manualmente configuração nativa (nginx -t, php-fpm8.3 -t ou php-fpm8.5 -t), serviços, login, uploads, APIs, integrações, negativa de escrita no código e scripts bloqueados em uploads. PHP CLI não comprova as configurações do FPM. Não deixar diagnóstico público.
