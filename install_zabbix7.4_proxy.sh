...
VERSION="1.1"
# ==============================================================================
# SCRIPT DE INSTALAÇÃO E CONFIGURAÇÃO DO ZABBIX PROXY 7.4 - UBUNTU 26.04
# COM SUPORTE A SQLITE3, AGENTE ZABBIX, SNMP E CRIPTOGRAFIA PSK
# ==============================================================================
# Execução recomendada (copiar e colar comando único):
# wget https://raw.githubusercontent.com/Lucasolidev/Scripts/main/install_zabbix7.4_proxy.sh -O install_zabbix7.4_proxy.sh && sudo chmod +x install_zabbix7.4_proxy.sh && sudo ./install_zabbix7.4_proxy.sh
# ==============================================================================
...
LOG_FILENAME="relatorio_install_zabbix7.4_proxy_${LOG_TIMESTAMP}.log"
LOG_LATEST="relatorio_install_zabbix7.4_proxy_latest.log"

RUNTIME_DIR=$(mktemp -d -p /tmp install_zabbix74_proxy.XXXXXXXX) || exit 1
...
REPO_DEB="zabbix-release_latest_7.4+ubuntu26.04_all.deb"
REPO_URL="https://repo.zabbix.com/zabbix/7.4/release/ubuntu/pool/main/z/zabbix-release/${REPO_DEB}"

log_info "Baixando pacote do repositório oficial Zabbix 7.4 (Ubuntu 26.04)..."
curl -fsSL "$REPO_URL" -o "${RUNTIME_DIR}/${REPO_DEB}"
dpkg -i "${RUNTIME_DIR}/${REPO_DEB}" > /dev/null 2>&1
log_success "Repositório Zabbix 7.4 adicionado."
...
echo -e "  ${FG_GREEN}${BOLD}✔ INSTALAÇÃO DO ZABBIX PROXY 7.4 CONCLUÍDA!${NC}\n"
echo -e "  ${DIM}────────────────────────────────────────────────────────────────${NC}"
