...
| :--- | :--- |
| `/var/lib/docker/` | Diretório padrão de armazenamento do Docker (contêineres, imagens, volumes e redes). |
| `/etc/docker/daemon.json` | Arquivo de configuração global do daemon Docker (DNS, drivers de log, registries). |
| `/var/run/docker.sock` | Socket Unix utilizado pela CLI do Docker para se comunicar com o daemon local. |
| `docker-compose.yml` | Arquivo padrão de especificação e orquestração multi-contêiner do Compose. |
| `~/.docker/config.json` | Configurações do cliente Docker do usuário (credenciais de login e plugins). |

---

## 🚀 2. Ciclo de Vida e Operação de Contêineres
...
## 🖼️ 3. Gerenciamento de Imagens e Limpeza de Disco
...
## 💾 4. Gerenciamento de Volumes Persistentes e Redes
...
## 🐙 5. Gerenciamento com Docker Compose (`docker-compose.yml`)
...
## 📄 6. Modelo Prático de `docker-compose.yml` (Nginx + MySQL + WordPress)
...
services:
  db:
    ...
    environment:
      ...
      MYSQL_USER: wp_user
      MYSQL_PASSWORD: <senha_usuario_banco>
    volumes:
  ...
  wordpress:
    ...
    environment:
      ...
      WORDPRESS_DB_PASSWORD: <senha_usuario_banco>
